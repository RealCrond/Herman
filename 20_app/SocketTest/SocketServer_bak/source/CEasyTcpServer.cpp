...
int CEasyTcpServer::Init()
{
	...
	{
		printf("Fail to load socket version2.2\n");
	}
#endif // _WIN32
	...
	for (unsigned int index = 0; index < m_vcScClient.size(); index++)
	{
		if ( FD_ISSET(m_vcScClient[index],&fdRead) )
		{
			...
			else
			{
				...
				if (duration >= 1.0)
				{
					printf("duration<%f>, server socket<%d>, send datasize<%lld KB>\n", duration, m_scServer, _nCount);
					_nCount = 0;
					_begin = std::chrono::high_resolution_clock::now();
				}
				std::this_thread::sleep_for(std::chrono::milliseconds(10));
				OnNetMsg(m_vcScClient[index], recvBuff);
			}
		}
	}
...
int CEasyTcpServer::OnNetMsg(SOCKET sc, char* recvBuff)
{
	...
	case emQuitCmd:
	{
		...
		int res = rand() % 15;
		ptLogoutRes->res = res;
		strcpy(ptLogoutRes->data, "Quit successfully!!!");
